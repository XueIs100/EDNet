...
def make_dataset(root_path, annotation_path, subset, n_samples_for_each_video, end_second, sample_duration, fold):
    # 1. 首先调用`load_annotation_data`函数加载注释数据，并赋值给变量`data`。
    data = load_annotation_data(annotation_path, fold)

    # 2. 调用`get_video_names_and_annotations`函数获取视频名称列表和注释列表，并分别赋值给`video_names`和`annotations`。
    ...
    for i in range(len(video_names)):
        ...
        print('我的调试：video_path和文件夹内数量（车内-2得到帧数量）：', video_path, n_frames)
