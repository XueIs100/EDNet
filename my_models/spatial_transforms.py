...
'''
Image 模块提供了处理图像的核心功能，可以用来加载、保存、创建缩略图、处理图像格式等。
而 ImageOps 模块则提供了一些基本的图像操作函数，例如调整大小、裁剪、旋转、翻转等
'''
try:
    import accimage
except ImportError:
...
class DriverCenterCrop(object):
    """Crop the area where the driver is in the center
    Args:
        scales: cropping scales of the original size
        size: size of the smaller edge
        interpolation: Default: PIL.Image.BILINEAR
    ...
    def __call__(self, img):
        ...
        img = img.crop((400, 100, img.size[0]-400, img.size[1]-100))

        return img.resize((self.size, self.size), self.interpolation)

    def randomize_parameters(self):
        self.tl_x = random.randint(0, 200)
        self.tl_y = random.randint(0, 200)
